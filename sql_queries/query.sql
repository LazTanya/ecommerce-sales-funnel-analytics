SELECT * FROM `phonic-axle-397713.city_data.Funnel_Analysys` LIMIT 1000
...
WITH funnel_stages AS (
  ...
  WHERE event_date >= TIMESTAMP_SUB(
    (SELECT MAX(event_date) FROM `phonic-axle-397713.city_data.Funnel_Analysys`),
    INTERVAL 30 DAY
)
)
...
SELECT 
  ...
  total_revenue / total_buyers AS revelue_per_buyer,
  total_revenue / total_visitors AS revelue_per_visitor
